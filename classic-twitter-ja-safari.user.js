// ==UserScript==
// @name         Classic Twitter for tweet.app - Japanese (Safari + Stay)
// @namespace    https://tweet.app/
// @version      6.2.5
// @description  iPhone/iPad Safari + Stay向け。tweet.appを旧Twitter風に日本語化。表示名、Founder Number、★お気に入り、リツイート、通知、返信通知補完、ローカルミュート、お気に入り一覧に対応。テーマには干渉しません。
...
(() => {
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      [data-testid="tweet-like-action"]::before{content:"☆";display:inline-block;font:700 23px/18px Arial,sans-serif;transform:translateY(-1px);transform-origin:center}
      [data-testid="tweet-like-action"][aria-pressed="true"]::before,[data-testid="tweet-like-action"].text-pink-500::before{content:"★";color:#ffac33!important}
      [data-testid="tweet-like-action"][aria-pressed="true"],[data-testid="tweet-like-action"].text-pink-500{color:#ffac33!important}
      [data-testid="tweet-like-action"]:hover{color:#ffac33!important;background:rgba(255,172,51,.12)!important}
      [data-testid="tweet-like-action"].ct-star-pop::before{animation:ctStarPop .32s cubic-bezier(.34,1.56,.64,1)}
      @keyframes ctStarPop{0%{transform:translateY(-1px) scale(.72) rotate(-8deg);opacity:.55}48%{transform:translateY(-1px) scale(1.3) rotate(6deg)}74%{transform:translateY(-1px) scale(.94) rotate(-2deg)}100%{transform:translateY(-1px) scale(1)}}
      .ct-founder,.ct-profile-founder{display:inline-flex;align-items:center;margin-left:4px;font-size:12px;line-height:18px;font-weight:700;white-space:nowrap;opacity:.72}
      .ct-profile-founder{font-size:13px;opacity:.78}
      .ct-twitter-logo{width:28px!important;height:28px!important;object-fit:contain!important;display:block!important}
      ...
      .ct-notification-fav-icon::after{content:"★";position:absolute!important;left:50%!important;top:50%!important;transform:translate(-50%,-50%)!important;color:#ffac33!important;font:700 27px/1 Arial,sans-serif!important;pointer-events:none!important}
      #ct-favorites-tab{position:relative;z-index:3;flex:1 1 0!important;min-width:0!important;border:0;border-bottom:2px solid transparent!important;background:transparent;color:inherit;font:600 14px/1.2 system-ui,-apple-system,"Segoe UI",sans-serif;cursor:pointer;padding:0 10px}
      #ct-favorites-tab:hover{background:rgba(127,127,127,.08)}
      #ct-favorites-tab[data-active="1"]{color:#1d9bf0;border-bottom-color:#1d9bf0!important}
      #ct-favorites-panel,#ct-reply-panel{position:fixed;z-index:2147482000;overflow:auto;overscroll-behavior:contain;background:inherit;color:inherit;border:1px solid rgba(127,127,127,.28);box-shadow:0 10px 35px rgba(0,0,0,.25)}
      #ct-favorites-panel{border-radius:0 0 12px 12px}
      #ct-reply-panel{border-radius:14px;max-height:min(48vh,460px)}
      .ct-local-head{position:sticky;top:0;z-index:2;display:flex;justify-content:space-between;gap:12px;padding:12px 16px;border-bottom:1px solid rgba(127,127,127,.28);background:inherit;color:inherit}
      .ct-local-title{font-weight:800}
      .ct-local-note{font-size:11px;opacity:.6;margin-top:2px}
      .ct-local-close{border:0;background:transparent;color:inherit;cursor:pointer;font-size:20px;padding:3px 8px;border-radius:999px}
      .ct-local-close:hover{background:rgba(127,127,127,.14)}
      .ct-local-card{display:flex;gap:10px;padding:12px 16px;border-bottom:1px solid rgba(127,127,127,.28);cursor:pointer}
      .ct-local-card:hover{background:rgba(127,127,127,.08)}
      .ct-local-avatar{width:40px;height:40px;border-radius:999px;object-fit:cover;flex:0 0 auto;background:rgba(127,127,127,.25)}
      .ct-local-main{min-width:0;flex:1}
      .ct-local-meta{display:flex;gap:4px;flex-wrap:wrap;align-items:center;font-size:13px;line-height:18px}
      .ct-local-name{font-weight:800}
      .ct-local-handle,.ct-local-time{opacity:.62}
      .ct-local-text{margin-top:3px;white-space:pre-wrap;overflow-wrap:anywhere;font-size:14px;line-height:20px}
      ...
      #ct-reply-badge{position:fixed;z-index:2147483000;min-width:18px;height:18px;padding:0 5px;border-radius:999px;background:#1d9bf0;color:#fff;font:700 11px/18px Arial;text-align:center;pointer-events:none}
      @media(max-width:700px){#ct-favorites-panel,#ct-reply-panel{max-width:calc(100vw - 12px)!important}.ct-local-head,.ct-local-card{padding-left:12px;padding-right:12px}#ct-favorites-tab{font-size:13px;padding-left:6px;padding-right:6px}}
    `;
  ...
  function translateNotification(t) {
    ...
    let m;
    const many = [
      [/^(.+?)\s+and\s+(\d+)\s+others?\s+(?:liked|favorited) your (?:post|tweet)$/i,(a,n)=>`${actorJP(a)}、そのほか${n}人があなたのツイートをお気に入りに登録しました`],
      [/^(.+?)\s+and\s+(\d+)\s+others?\s+(?:reposted|retweeted) your (?:post|tweet)$/i,(a,n)=>`${actorJP(a)}、そのほか${n}人があなたのツイートをリツイートしました`],
      [/^(.+?)\s+and\s+(\d+)\s+others?\s+followed you$/i,(a,n)=>`${actorJP(a)}、そのほか${n}人があなたをフォローしました`],
      [/^(.+?)\s+and\s+(\d+)\s+others?\s+mentioned you(?: in a (?:post|tweet))?$/i,(a,n)=>`${actorJP(a)}、そのほか${n}人があなたを@ツイートしました`],
      [/^(.+?)\s+and\s+(\d+)\s+others?\s+replied to (?:your (?:post|tweet)|you)$/i,(a,n)=>`${actorJP(a)}、そのほか${n}人があなたのツイートに返信しました`]
    ];
    for (const [re, fn] of many) {
      if ((m = t.match(re))) return fn(m[1], m[2]);
    }
    const one = [
      [/^(.+?)\s+(?:liked|favorited) your (?:post|tweet)$/i,a=>`${actorJP(a)}があなたのツイートをお気に入りに登録しました`],
      [/^(.+?)\s+(?:reposted|retweeted) your (?:post|tweet)$/i,a=>`${actorJP(a)}があなたのツイートをリツイートしました`],
      [/^(.+?)\s+followed you$/i,a=>`${actorJP(a)}があなたをフォローしました`],
      [/^(.+?)\s+mentioned you(?: in a (?:post|tweet))?$/i,a=>`${actorJP(a)}があなたを@ツイートしました`],
      [/^(.+?)\s+replied to your (?:post|tweet)$/i,a=>`${actorJP(a)}があなたのツイートに返信しました`],
      [/^(.+?)\s+replied to you$/i,a=>`${actorJP(a)}があなたに返信しました`],
      [/^(.+?)\s+(?:liked|favorited) your reply$/i,a=>`${actorJP(a)}があなたの返信をお気に入りに登録しました`],
      [/^(.+?)\s+(?:reposted|retweeted) your reply$/i,a=>`${actorJP(a)}があなたの返信をリツイートしました`]
    ];
  ...
  function translateTextNode(node) {
    ...
    if (location.pathname.startsWith('/notifications')) {
      let m;
      if (/^and$/i.test(t)) out = 'と';
      else if ((m = t.match(/^and\s+(\d+)\s+others?$/i))) out = `とそのほか${m[1]}人`;
      else if ((m = t.match(/^(\d+)\s+others?$/i))) out = `そのほか${m[1]}人`;
    }
    if (!out) out = translateNotification(t) || JP.get(t) || dynamicJP(t);
  ...
  function notificationTextNodes(root = document) {
    const main = root instanceof Element && root.matches('main') ? root : root.querySelector?.('main') || document.querySelector('main');
    if (!main) return [];
  ...
  function patchNotificationConnectors(root = document) {
    ...
    for (const node of nodes) {
      ...
      if (/^others?$/i.test(t)) {
        ...
        if (/^\d+$/.test(pt)) {
          const n = pt;
          if (prev.nodeType === Node.TEXT_NODE) prev.nodeValue = `そのほか${n}人`;
          else if (prev instanceof Element && !prev.children.length) prev.textContent = `そのほか${n}人`;
          else node.nodeValue = `そのほか${n}人`;
          if (clean(node.nodeValue || '') !== `そのほか${n}人`) node.nodeValue = '';
        }
      }
      if (/^\d+$/.test(t)) {
        ...
        if (/^others?$/i.test(nt)) {
          ...
          else if (next instanceof Element && !next.children.length) next.textContent = '';
        }
      }
    }
    ...
    for (const el of main.querySelectorAll('span,small,strong,a,button,div,p')) {
      ...
      if (!t || t.length > 64) continue;
      let m;
      if ((m = t.match(/^and\s+(\d+)\s+others?$/i))) el.textContent = `とそのほか${m[1]}人`;
      else if ((m = t.match(/^(\d+)\s+others?$/i))) el.textContent = `そのほか${m[1]}人`;
      else if (/^and$/i.test(t)) el.textContent = 'と';
    }
  }

  function isMobileSafari() {
    const ua = navigator.userAgent || '';
    return /Safari/i.test(ua) && !/CriOS|Chrome|FxiOS|EdgiOS|OPiOS/i.test(ua) && (/iPhone|iPad|iPod/i.test(ua) || (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1));
  }

  function patchSafariTopTitle() {
    if (!isMobileSafari()) return;
    const candidates = [...document.querySelectorAll('header h1,header h2,header span,main h1,main h2,[role="banner"] h1,[role="banner"] h2,[role="banner"] span')];
    for (const el of candidates) {
      if (!el.isConnected || el.children.length) continue;
      const rect = el.getBoundingClientRect();
      if (rect.top < 0 || rect.top > 140 || rect.width <= 0 || rect.height <= 0) continue;
      if (clean(el.textContent) === 'ツイート') el.textContent = 'Twitter';
    }
  }

  function patchInputs(root = document) {
    const list = [];
    if (root instanceof Element && root.matches('input[placeholder],textarea[placeholder]')) list.push(root);
  ...
  function hideAffiliation() {
    document.querySelectorAll('span,p,div,a,small').forEach(el => {
      if (el.isConnected && !el.children.length && /^Not Affiliated with X$/i.test(clean(el.textContent))) el.style.setProperty('display', 'none', 'important');
    });
  }
  ...
  function patchBrand() {
    document.querySelectorAll('img').forEach(img => {
      ...
      let path = '';
      try { path = new URL(rawSrc, location.href).pathname; } catch { path = rawSrc; }
  ...
  function requestJSON(url, headers = {}) {
    if (typeof GM_xmlhttpRequest === 'function') {
      return new Promise(resolve => {
        GM_xmlhttpRequest({
          ...
          onload: response => {
            try { resolve(response.status >= 200 && response.status < 300 ? JSON.parse(response.responseText) : null); }
            catch { resolve(null); }
          },
    ...
    return fetch(url, { headers: { Accept: 'application/json', ...headers } }).then(r => r.ok ? r.json() : null).catch(() => null);
  }
  ...
  function articleAuthor(article) {
    ...
    for (const el of article.querySelectorAll('button[aria-label],a[href],span,div')) {
      ...
      if (user) return user;
      if (!el.children.length && (m = clean(el.textContent).match(/^@([A-Za-z0-9_.-]{1,80})$/))) return validUser(m[1]);
    }
    return null;
  }
  ...
  function findAuthorLeaf(article, username) {
    ...
    return [...article.querySelectorAll('button,span,a,div,strong')].find(el => {
  ...
  function saveMuted() { saveJSON(KEY.muted, [...muted].sort()); }
  ...
  async function patchRetweetRows(root = document) {
    const leaves = [];
    root.querySelectorAll?.('span,div,p').forEach(el => { if (!el.children.length) leaves.push(el); });
    for (const el of leaves) {
      const t = clean(el.textContent);
      const m = t.match(/^@?([A-Za-z0-9_.-]{1,80})\s+(?:reposted|retweeted)$/i);
      if (!m) continue;
      const username = validUser(m[1]);
      const user = username ? await fetchProfile(username) : null;
      if (!user || !el.isConnected) continue;
      el.textContent = `${user.displayName || user.name || username}さんがリツイートしました`;
    }
  }
  ...
  function patchProfileMute() {
    ...
    const main = document.querySelector('main') || document;
    const existing = [...main.querySelectorAll('.ct-profile-mute')];
    const old = existing.shift() || null;
    existing.forEach(el => el.remove());
    if (!username) { old?.remove(); return; }
    const ref = [...main.querySelectorAll('button')].find(button => !button.closest('article') && !button.classList.contains('ct-profile-mute') && /^(Follow|Unfollow|フォロー|フォロー解除)$/i.test(clean(button.textContent)));
    if (!ref) { old?.remove(); return; }
    ...
    const label = isMuted(username) ? 'ミュート解除' : 'ミュート';
  ...
  function articleText(article) {
    return [...article.querySelectorAll('p,div[dir="auto"],span')]
      .filter(el => !el.children.length)
      .map(el => clean(el.textContent))
      .filter(t => t.length > 2 && !/^@/.test(t) && !/^\d+[smhd]$/.test(t) && !JP.has(t))
  ...
  function snapshotFavorite(article) {
    ...
    const href = [...article.querySelectorAll('a[href]')].map(el => el.href).find(h => /(post|status|tweet)\//i.test(h)) || '';
    return { id, username, name, text: articleText(article), avatar: articleAvatar(article), href, savedAt: Date.now() };
  }
  ...
  function saveFavorite(item) {
    if (!item?.id) return;
    saveJSON(KEY.favorites, [item, ...loadFavorites().filter(x => x.id !== item.id)].slice(0, 500));
  }
  ...
  function patchFavoriteButtons(root = document) {
    root.querySelectorAll?.('[data-testid="tweet-like-action"]').forEach(button => {
      ...
      button.title = liked ? 'お気に入りを解除' : 'お気に入り';
      const aria = button.getAttribute('aria-label') || '';
      button.setAttribute('aria-label', aria.replace(/^Like,/i, 'お気に入り,').replace(/^Unlike,/i, 'お気に入りを解除,'));
      if (liked) {
        const article = button.closest('article');
  ...
  function findRepostTab() {
    if (location.pathname !== '/profile') return null;
    return [...document.querySelectorAll('main a,main button,main [role="tab"]')].find(el => /^(Reposts|Retweets|リツイート)$/i.test(clean(el.textContent))) || null;
  }
  ...
  function patchFavoriteProfileTab() {
    ...
    const ref = findRepostTab();
    if (!ref) { tab?.remove(); return; }
    if (!tab) {
      tab = document.createElement('button');
      tab.id = 'ct-favorites-tab';
      tab.type = 'button';
      tab.textContent = 'お気に入り';
  ...
  function localCard(item, reply = false) {
    ...
    if (reply) {
      ...
      kicker.textContent = 'あなたのツイートに返信しました';
    ...
    name.textContent = item.name || item.authorName || item.username || item.authorUsername || '';
  ...
  function closeFavoritesPanel() { document.getElementById('ct-favorites-panel')?.remove(); }

  function renderFavoritesPanel() {
    if (!favoritesActive || location.pathname !== '/profile') { closeFavoritesPanel(); return; }
    const r = centerRect();
    if (!r) return;
    ...
    const safeWidth = Math.min(Math.round(r.width), Math.max(280, window.innerWidth - 12));
    const safeLeft = Math.max(6, Math.min(Math.round(r.left), window.innerWidth - safeWidth - 6));
    ...
    panel.innerHTML = '';
    panel.appendChild(panelHead('★ お気に入り', 'このブラウザだけに保存されます', () => {
      favoritesActive = false;
    ...
    for (const item of data) {
      ...
      row.onclick = () => { if (item.href) location.href = item.href; };
  ...
  function patchNotifications(root = document) {
    if (!location.pathname.startsWith('/notifications')) return;
    patchNotificationConnectors(root);
    for (const el of notificationLeaves(root)) {
      const t = clean(el.textContent);
      if (!t) continue;
      const out = translateNotification(t) || JP.get(t) || dynamicJP(t);
      if (out && out !== t) el.textContent = out;
      const finalText = out || t;
      if (/お気に入りに登録しました/.test(finalText)) {
  ...
  function authJSON(path, auth) {
    ...
    return requestJSON(path.startsWith('http') ? path : API_ORIGIN + path, { Authorization: `Bearer ${auth.token}` });
  }
  ...
  const replyCount = post => Number(post?.replyCount ?? post?.comments ?? post?.commentCount ?? post?.repliesCount ?? post?.reply_count ?? 0) || 0;
  const author = post => validUser(post?.authorUsername || post?.authorHandle || post?.author?.username || post?.username);
  const postText = post => String(post?.text || post?.body || post?.content || post?.replyText || '');
  const postName = post => String(post?.authorName || post?.actorName || post?.author?.displayName || author(post) || '');
  const postAvatar = post => String(post?.authorAvatar || post?.actorAvatar || post?.author?.avatarUrl || post?.avatarUrl || '');
  ...
  function saveReplyNotice(reply, parentId = '') {
    ...
    saveJSON(KEY.replyNotices, [record, ...list.filter(x => x.id !== id)].sort((a,b)=>new Date(b.createdAt || b.detectedAt)-new Date(a.createdAt || a.detectedAt)).slice(0,100));
  }

  function markReplyRead(id) {
    const list = loadReplyNotices();
    list.forEach(item => { if (item.id === id) item.read = true; });
  ...
  async function replyWatchTick() {
    ...
    try {
      ...
      const myHandle = validUser(me?.username || me?.handle || me?.user?.username || me?.profile?.username);
      ...
      for (const notification of items(notificationsJson, ['notifications'])) {
        const type = String(notification?.type || notification?.eventType || notification?.kind || notification?.notificationType || '').toUpperCase();
        const message = String(notification?.message || notification?.text || '');
        if (!type.includes('REPLY') && !/replied to|返信/i.test(message)) continue;
        ...
        const username = validUser(notification?.actorHandle || notification?.actorUsername || notification?.actor?.username || author(object));
        if (id && username) {
          saveReplyNotice({
            ...object,
            id,
            authorUsername: username,
            authorName: notification?.actorName || notification?.actorDisplayName || notification?.actor?.displayName || postName(object) || username,
            authorAvatar: notification?.actorAvatar || notification?.actor?.avatarUrl || postAvatar(object),
            text: notification?.replyText || notification?.postText || postText(object),
            createdAt: notification?.createdAt || notification?.created_at || postCreated(object)
      ...
      const parents = [...items(postsJson, ['posts']), ...items(repliesJson, ['replies'])]
        .filter(post => postId(post) && replyCount(post) > 0)
        .sort((a,b)=>new Date(postCreated(b) || 0)-new Date(postCreated(a) || 0))
        .slice(0,24);
      ...
      console.debug('[Classic Twitter JP reply watcher]', error);
    } finally {
      replyBusy = false;
    }
  }

  function closeReplyPanel() { document.getElementById('ct-reply-panel')?.remove(); }

  function renderReplyPanel() {
    if (!location.pathname.startsWith('/notifications')) { closeReplyPanel(); return; }
    const data = loadReplyNotices().filter(item => !item.read);
    if (!data.length) { closeReplyPanel(); return; }
    const r = centerRect();
    if (!r) return;
    ...
    const safeWidth = Math.min(Math.max(280, Math.round(r.width - 24)), Math.max(280, window.innerWidth - 12));
    const safeLeft = Math.max(6, Math.min(Math.round(r.left + 12), window.innerWidth - safeWidth - 6));
    panel.style.left = `${safeLeft}px`;
    panel.style.top = `${Math.max(90, Math.round(r.top + 80))}px`;
    panel.style.width = `${safeWidth}px`;
    panel.innerHTML = '';
    panel.appendChild(panelHead('↩ 新しい返信', 'tweet.app本体で表示されない返信を補完しています', () => {
  ...
  function patchReplyBadge() {
    ...
    const link = [...document.querySelectorAll('a,button')].find(el => /^(Notifications|通知)$/i.test(clean(el.textContent)));
    if (!count || !link) { badge?.remove(); return; }
    if (!badge) {
    ...
    badge.style.left = `${Math.max(4, Math.min(Math.round(r.right - 8), window.innerWidth - 24))}px`;
  ...
  function scan(root = document) {
    try {
      ...
      console.debug('[Classic Twitter JP Safari]', error);
    }
  }
  ...
  console.log('🐦 Classic Twitter JP Safari + Stay v6.2.5 loaded');
})();
