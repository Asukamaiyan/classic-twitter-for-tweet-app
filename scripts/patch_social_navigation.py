...
TARGETS = [
    (Path('classic-twitter-ja.user.js'), '6.3.0'),
    (Path('classic-twitter-ja-safari.user.js'), '6.3.3'),
    (Path('classic-twitter-en.user.js'), '6.2.6-en'),
]
...
for path, version in TARGETS:
    ...
    if 'function patchRetweetNavigation(' not in s:
        pos = s.find('  function scan(')
        if pos < 0:
            raise RuntimeError(f'scan marker not found: {path}')
        s = s[:pos] + HELPER + '\n' + s[pos:]

    call_anchor = s.rfind('patchRetweetRows(')
    if call_anchor < 0:
        raise RuntimeError(f'patchRetweetRows call not found: {path}')
    semi = s.find(';', call_anchor)
    if semi < 0:
        raise RuntimeError(f'patchRetweetRows terminator not found: {path}')

    insertion = '\n\n      patchRetweetNavigation(root);\n      patchQuotedTweets(root);\n      removeInlineFollowBadges(root);'
    if 'patchRetweetNavigation(root);' not in s[call_anchor:call_anchor + 1200]:
